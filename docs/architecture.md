...
- **Seguridad (prioridad máxima)**: RLS por tenant + `patient_access` (visibilidad clínica) + rol sin bypassrls + FORCE RLS (ya implementado). Contenido clínico **nunca a logs ni a prompts sin filtrar**. Audit de accesos clínicos.
- **Cifrado de campo del contenido clínico (parte del diseño completo, NO diferido):** el texto libre clínico (`clinical_notes.content` + `input_bullets`, campos de texto libre de `clinical_files.values`) se cifra a nivel de aplicación con **envelope encryption** — clave maestra en KMS/secret de Railway (**nunca en la DB**) → DEK por tenant → AES-256-GCM. El backend descifra **al vuelo en cada request autorizado** (profesional con `patient_access` o el agente de notas, en memoria, nunca a logs); para el profesional es **transparente** (no maneja claves). Protege contra robo de la DB (dump/backup/SQLi/réplica). NO protege contra compromiso total del server (clave + DB) — nivel superior (KMS dedicado / claves por usuario) post-MVP.
  - Demográficos (nombre/teléfono) quedan consultables (no cifrados) para el listado; diseño completo previsto con **blind index** si se decide cifrarlos — construcción escalonable, no placeholder permanente.
  - Trade-off aceptado: no hay búsqueda full-text del texto clínico cifrado (no es una feature ofrecida).
- **Portabilidad / retención (Ley 26.529)**: el profesional es "depositario" de la HC por **10 años**. Export **durable e imprimible** (PDF/A + datos crudos JSON) — es una **lectura autorizada que descifra al vuelo**, no un repositorio en claro aparte. Periódico y al darse de baja o si Conflur cierra. Borrado siempre lógico, nunca físico. Diferencial de confianza.
- **Backups (parte del diseño completo):** backup de datos (Neon **PITR**, requiere plan pago — el free tier no alcanza para 10 años) **+ backup seguro de la clave maestra (key escrow)** — sin la clave, un restore de datos cifrados es inservible y se perdería la HC. Política de retención explícita a 10 años.
- **Frontera de construcción (secuenciar, no mutilar):** para el **demo** (pacientes de prueba propios) no hace falta el cifrado. **Antes de pacientes reales de psicólogos reales:** cifrado de contenido clínico + backups (PITR + key escrow) + postura legal (DPA/ToS/breach). El diseño es completo; lo que se escalona es *cuándo* se construye, con frontera clara = "dato real de paciente".
...
**Decisión D19 [2026-06-22, revisada 2026-06-23]:** seguridad clínica como prioridad no negociable (dos capas ya implementadas) + **export durable de fichas** (PDF/A + JSON) como requisito de primera clase por la Ley 26.529. Consentimiento digital parqueado (papel presencial).

**Decisión D22 [2026-06-23]:** el **cifrado de campo del contenido clínico** (envelope encryption, clave server-side, descifrado transparente para acceso/export autorizado) deja de estar "diferido" → es **parte del diseño completo** y se construye **antes de onboardear pacientes reales** (no para el demo). Junto con **backups (Neon PITR + key escrow) + retención 10 años + postura legal (DPA/ToS/breach)**. Frontera de build = "dato real de paciente": secuenciar la construcción ✓, mutilar el diseño ✗.

**Decisión D23 [2026-06-23]:** primer vertical **confirmado = psicólogos**, re-validado contra el riesgo legal/sensibilidad. Razón: mercado mucho mayor que fonoaudiología (CAC más bajo), sin insumos (adopción de mínima fricción) y máximo valor del diferenciador (notas narrativas). La mayor sensibilidad del dato se acepta y se mitiga con D22 (cifrado + backups + postura legal), no cambiando de vertical (todas las de salud comparten la obligación de la Ley 26.529).
